...
#epi1 = epi1[epi1$`Variant effect` %in% c("stop gained", "splice donor","splice acceptor","frameshift, feature elongation", "frameshift, feature truncation")]
epi1$count = 1
...
#epi2 = epi2[epi2$`Coding variant type` %in% c("stopgain_SNV", "canonical splice site_SNV","frameshift_insertion","frameshift_deletion")]
epi2$count = 1
...
#scz1=scz1[`Family history`=="."]
#scz1=scz1[as.numeric(`Proband age at onset of disease`)<20]
#scz1=scz1[`Proband gender`=="female"]
...
#scz2=scz2[FAMILY_HISTORY_MENTAL_ILLNESS=="FALSE"]
#scz2=scz2[as.numeric(PROBAND_AGE_OF_ONSET)<20]
#scz2=scz2[SEX.x=="Female"]
...
#ped = ped[n_in_fam==2]
#ped = ped[sex_matched==1]
#ped = ped[sex_of_proband=="female" & sex_matched==1 & Pheno=="control"]
...
#ssc = ssc[,c("Chr","Pos","Type","Pheno","SYMBOL","SampleID")]
ssc = ssc[!SYMBOL=="."]
ssc = ssc[!mut=="indel"]
ssc$count=1
...
#mssng = mssng[FAMILY_TYPE=="SPX"]
#mssng = mssng[!grepl("AGRE",COHORT)]
mssng = mssng[SEX=="M"]
mssng = mssng[!grepl("AffectedSibling",rel)]
#mssng=mssng[COHORT == "ASD: Genomes to Outcome Study"]
